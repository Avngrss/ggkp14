...
- Поиск не выполняет серверный запрос и не индексирует страницы. Формы ведут на статическую `search-results.html`.
- Два визуальных состояния одного шаблона: обычные результаты и `?state=empty&s=...`. Параметр `?s=` только отображается как текст.
